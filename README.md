...
* updateLock - Update dependencies from the lock file into the build directory. By default, this task does the same thing as the `generateLock` task except no dependencies will be removed from the lock file, only updated. This task also exposes an option `--dependencies` allowing the user to specify a comma-separated list, in the format `<group>:<artifact>`, of dependencies to update.
